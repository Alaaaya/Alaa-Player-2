...
- Static size/coupling measurements: `SyncManager` is 6,048 lines/169 functions; `OkHttpStalkerApiService` 2,767/124; `Media3PlayerEngine` 2,482/122; `PreferencesRepository` 2,324/169; `PlayerViewModel` 1,776 lines; `StalkerProvider` 1,611/88; `ProviderRepositoryImpl` 1,308 lines.
...
- **Classification:** Architectural concern
- **Severity:** High
- **Where:** `data/.../sync/SyncManager.kt` (6,048 lines, 169 functions, 27 constructor dependencies/created collaborators), plus `ProviderSyncWorker`, `XtreamIndexWorker`, `StalkerIndexWorker`, `ProviderRepositoryImpl`, `MovieRepositoryImpl`, and `SeriesRepositoryImpl`.
- **Current behavior:** The class owns provider locking/state publication, M3U downloading/parsing, Xtream and Stalker authentication/catalog logic, category staging, transaction commits, EPG decisions, retries/timeouts, incremental index job state, worker scheduling, error sanitization, metadata/status updates, and repair flows. It creates additional support objects internally while also exposing provider-specific public methods used by workers and UI/repositories.
